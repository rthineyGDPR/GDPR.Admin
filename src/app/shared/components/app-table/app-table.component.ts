...
import { DataService, ModalService } from 'src/app/services';
...
export class AppTableComponent implements OnInit {
  ...
  getData() {
    this.dataService.get<Array<any>>(this.options.apiUrl).subscribe(data => {
  ...
  edit(row, rowIndex) {
    // Form Template
    this.createOrEdit(row, rowIndex).then(
      () => {},
      () => {}
    );
  }
  delete(row, rowIndex) {
    this.modalService
      ...
      .then(
        () => {
          this.dataService
            .delete(`${this.options.apiUrl}/${row.id}`)
            .subscribe(() => {
              this.options.rows = this.options.rows.filter(
                x => x.id !== row.id
              );
  ...
  private createOrEdit(row = null, rowIndex = null): Promise<any> {
    ...
    function update(form) {
      if (form.valid) {
        this.dataService
          .put(`${this.options.apiUrl}/${row.id}`, { ...form.value })
          .subscribe(res => {
    ...
    function create(form) {
      if (form.valid) {
        this.dataService
          .post(this.options.apiUrl, { ...form.value })
          .subscribe(res => {
